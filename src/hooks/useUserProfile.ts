...
import { API_BASE_URL } from '@/constants/profile.constants';
...
export const useUserProfile = (navigate?: (path: string) => void): UseUserProfileReturn => {
  ...
  const uploadImage = async (file: File): Promise<void> => {
    ...
    try {
      ...
      if (responseData.data) {
        setUser((prev) => (prev ? { ...prev, ...responseData.data } : responseData.data));
      } else if (responseData.user) {
        setUser((prev) => (prev ? { ...prev, ...responseData.user } : responseData.user));
      } else if (responseData.image || responseData.avatar || responseData.photo) {
        setUser((prev) =>
          prev
            ? {
                ...prev,
                avatar: responseData.image || responseData.avatar || responseData.photo,
              }
            : null
        );
      }
