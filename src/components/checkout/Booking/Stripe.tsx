...
function StripeForm({ bookingId }: { bookingId: string | undefined }) {
    ...
    const fetchPaymentData = async () => {
      try {
        ...
        const response = await axios.post(
          `${BASE_URL}/create-payment-intent/${bookingId}`,
  ...
  if (!stripePromise || !clientSecret) return <p className="text-center text-red-500">Failed to load payment data</p>;

  return (
    <div className="max-w-md mx-auto p-4 bg-slate-50 rounded-2xl border">
      <h3 className="text-lg font-bold mb-4 text-slate-800">Enter your payment details</h3>
